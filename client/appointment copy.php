                ...
                <div class="dashboard-items search-items">
                    <div style="width:100%;">
                        ...
                        <div style="margin-bottom: 5px;" class="h3-search">
                         ...
                         <div class="h1-search">
                            <button style="font-size: 16px; margin-left: 4px;" class="login-btn btn-primary-soft btn pay-now-btn" data-appoid="' . $appoid . '" data-archi="' . $archiname . '">
                            <i class="fa-brands fa-paypal"></i>
                                ' . ($pay_status === "Pay Remaining Payment Now" ? "Pay Remaining Payment Now" : "Fully Paid ☑") . '
                            </button>
                        </div>
                        <br>
                    </div>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.querySelectorAll(".pay-now-btn").forEach(function (button) {
                button.addEventListener("click", function () {
                    var appoid = this.getAttribute("data-appoid");
                    var archi = this.getAttribute("data-archi");

                    // AJAX request to update the payment status in the database
                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "update-payment-status.php", true);
                    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4 && xhr.status == 200) {
                            // On success, show the modal and change button text to "Paid"
                            modal.style.display = "block";
                            button.textContent = "Fully Paid";
                            button.disabled = true; // Optionally disable the button after payment
                        }
                    };
                    xhr.send("appoid=" + appoid + "&archi=" + archi);
                });
            });
